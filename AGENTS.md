...
- Current state: playable prototype with original in-repo pixel art, a main menu, chunk spawning, elemental combat, enemies, projectiles, deflects, a compact top HUD, persistent best score saving, and restart flow
...
- The elemental cooldown strip is compact, vertical, top-left under the health bar, and animates in/out based on recent combat activity.
- Best score persists across launches.
...
- `internal/game/save.go`: best-score persistence helpers
...
- HUD icons now use element-shaped symbols rather than generic circles:
  - fire = flame
  - ice = snowflake
...
- Persistence is intentionally lightweight and currently stores best score in the user's config directory.
